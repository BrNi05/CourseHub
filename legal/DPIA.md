...
**Készült:** 2026.07.07. (ötödik verzió)

**Következő tervezett felülvizsgálat:** 2027.07.07., illetve minden olyan lényeges funkcionális, infrastrukturális, adatfeldolgozói vagy adatkezelési változás előtt, amely az érintetti kockázatokat növelheti.
...
- Biztonsági naplózás és peremvédelem.
...
- [EDPB Template for Data Protection Impact Assessment](https://www.edpb.europa.eu/system/files/2026-05/edpb_dpia_template_2026_v1_en.docx)
...
### 4.1 Az adatkezelés vizualizálása

Az alábbi diagramok megtekinthetők [itt](https://github.com/BrNi05/CourseHub/edit/main/legal/DPIA.md#4-az-adatkezel%C3%A9s-r%C3%B6vid-le%C3%ADr%C3%A1sa). A nyilak az adatáramlást jelölik!

#### 4.1.1. Általános Rendszerarchitektúra és Adatáramlás

Az alábbi folyamatábra a szolgáltatás fizikai és logikai határait, (részlegesen) az alkalmazott védelmi vonalakat (Cloudflare, MFA, OAuth2), valamint az önálló adatkezelők közötti adatmozgást mutatja be.

``` mermaid
flowchart TD
    User(("Érintett / Felhasználó"))
    
    subgraph Kliens_Kornyezet [Kliens Környezet]
        Browser["Böngésző / Vue3 SPA<br/>Helyi tárhely: helyi mentések"]
    end
    
    subgraph Hatarvedelem [Peremhálózat]
        CF["Cloudflare<br/>CDN / WAF"]
    end
    
    subgraph Szerver_Kornyezet [CourseHub Szerver - Adatkezelő]
        NestJS["NestJS Backend<br/>Kiszolgálás és üzleti logika"]
        Redis[("Redis Cache<br/>LRU / JWT Blacklist / MFA")]
        DB[("PostgreSQL<br/>Adatbázis")]
        Logs[("Fájlrendszer<br/>Biztonsági Logok / Hibajelentések")]
    end
    
    subgraph Kulso_Szolgaltatok [Külső Adatkezelők]
        Google["Google OAuth2<br/>Hitelesítés"]
        Discord["Discord Webhook<br/>Anonim riasztás / Admin MFA"]
    end

    User <-->|Interakció| Browser
    Browser <-->|HTTP kérés / válasz| CF
    CF <-->|HTTP kérés / válasz| NestJS
    
    Browser -.->|OAuth folyamat| Google
    NestJS <-->|Hitelesítés eredménye| Google
    
    NestJS <-->|Adatbázis műveletek| DB
    NestJS <-->|Gyorsítótárazás / JWT blacklist| Redis
    NestJS <-->|Helyi naplózás| Logs
    
    NestJS -->|Anonim riasztások / MFA token| Discord
```

#### 4.1.2. Hitelesítési Folyamat és Biztonsági Garanciák

A kiemelt jogosultságú (admin) hozzáférés védelme és a munkamenetek biztonsága (login CSRF védelem, JWT blacklist, stb.) érdekében a CourseHub szigorú, "Fail-Closed" elvre épülő hitelesítési láncot alkalmaz.

``` mermaid
sequenceDiagram
    actor User as Érintett (Felhasználó / Admin)
    participant Vue as Vue3 Kliens
    ...
    User->>Vue: 1. Bejelentkezés indítása
    Vue->>Google: 2. Átirányítás Google-hitelesítésre
    Google-->>Nest: 3. Callback
    Nest->>DB: 4. Felhasználó keresése / létrehozása
    Nest->>Nest: 5. Jogosultság ellenőrzése (isAdmin?)
    
    alt Ha a felhasználó ADMIN
        Nest->>Redis: 6a. MFA token generálása és mentése
        Nest->>Discord: 6b. MFA token küldése
        Note right of Nest: Fail-Closed: Ha a Discord hívás<br/>timeout-ol, a login megszakad!
    end
    
    Nest->>Nest: 7. JWT generálása
    Nest-->>Vue: 8. HttpOnly, Secure süti beállítása
    Vue->>Nest: 9. Védett API kérés (süti+MFA)
    Nest->>Nest: 10. JWT 'jti' és aláírás ellenőrzése
    Nest->>Redis: 11.  O(1) JWT blacklist ellenőrzése
    Nest->>Nest: 12. MFA ellenőrzése
    Nest->>DB: 13. Adat lekérdezése / módosítása
    Nest-->>Vue: 14. API válasz
```
...
- Napi ping helyi deduplikációs adatai. (2026.07.07. után már nem generálódik.)
...
**Megjegyzés: A rendszer a GDPR 9. cikke szerinti különleges adatot nem kezel.**
...
| Korlátozott tárolhatóság                   | Automatikus inaktivitási törlés, javaslat- és logrotáció, kurzuscsomag inaktivitási törlés.                                                 |
...
A rendszer tervezése során az [OWASP Top 10](https://owasp.org/Top10/2025/) és [OWASP ASVS](https://github.com/OWASP/ASVS/raw/v5.0.0/5.0/OWASP_Application_Security_Verification_Standard_5.0.0_en.pdf) irányelvek figyelembevételével kerültek kialakításra a fő biztonsági kontrollok, így különösen a hitelesítés, jogosultságkezelés, munkamenet-kezelés, bemenet validáció és naplózás.
...
Az érintettek a CourseHub [adatkezelési tájékoztatója](https://github.com/BrNi05/CourseHub/blob/main/legal/privacy.md) alapján gyakorolhatják:
...
A preventív intézkedéseken túl az Adatkezelő incidenskezelési tervvel is rendelkezik. Egy esetleges adatvédelmi incidens (pl. jogosulatlan hozzáférés, adatszivárgás) észlelése esetén az Adatkezelő haladéktalanul megkezdi a kivizsgálást, az esetleges sérülékenység javítását, valamint a kompromittált adatkörök és eszközök felmérését.

- **Hatósági bejelentés:** Amennyiben az incidens valószínűsíthetően kockázattal jár a természetes személyek jogaira és szabadságaira nézve, az Adatkezelő indokolatlan késedelem nélkül, legkésőbb a tudomásszerzéstől számított 72 órán belül bejelenti azt a NAIH felé.

- **Érintettek tájékoztatása:** Ha az incidens valószínűsíthetően magas kockázattal jár a természetes személyek jogaira és szabadságaira nézve, az Adatkezelő haladéktalanul, közvetlenül (e-mailben) vagy nyilvános közlemény útján tájékoztatja az Érintetteket.

- **Technikai mitigáció:** Incidens esetén minden szükséges lépést megtesz az Adatkezelő, hogy a titkosság, az integritás és a rendelkezésre állás mihamarabb biztosítva legyen.

## 15. Érintettek véleményének figyelembevétele
...
## 16. Kockázatértékelési módszertan
...
## 17. Kockázatok, meglévő intézkedések és fennmaradó kockázat

| #   | Kockázat                                                                             | Lehetséges érintetti hatás                                                                                  | Kiinduló kockázat    | Megtett intézkedések                                                                                                                                                                                  | Fennmaradó kockázat |
| --- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| 1   | Szerveroldali jegyadatokhoz való jogosulatlan hozzáférés                             | Jegyek, tanulmányi előmenetel, szak vagy intézmény illetéktelen megismerése, szégyen és reputációs kockázat | 3 x 4 = magas        | Auth süti HttpOnly/Secure, saját adat csak autentikált felhasználónak, admin endpointok külön védve, rate limiting, HTTPS, DB hozzáférés korlátozása, törlési lehetőség, méretkorlát, logout          | 1 x 4 = alacsony    |
| 2   | Admin jogosultság hibás kezelése vagy admin fiók kompromittálódása                   | Több felhasználó adataihoz való hozzáférés, technikai adatok megismerése                                    | 4 x 5 = nagyon magas | Admin jogosultság adatbázisban, admin endpointok szigorú throttlinggal, admin hozzáférés szűk körű, böngészőben admin süti nem tárolt, HS384 algo., 30 perces access token, azonnali értesítések, MFA | 1 x 5 = közepes     |
| 3   | XSS, kliensoldali támadás vagy figyelmetlenség miatt helyi adatok szivárgása         | Helyi jegy- és kurzusadatok illetéktelen hozzáférése az adott eszközön                                      | 3 x 4 = magas        | CSP, script nonce, `script-src-attr` tiltás, referrer policy, nincs auth token localStorage-ban, lejárt munkamenet figyelmeztetések, helyi mentés alapértelmezett törlése                             | 2 x 4 = közepes     |
...
| 6   | Túl hosszú megőrzés, elfelejtett cache vagy elévült biztonsági mentések              | Már nem szükséges hallgatói adatok fennmaradása, törlés véglegességének hiánya                              | 3 x 4 = magas        | Inaktív felhasználók törlése, jegyadatok törlési lehetősége, cache invalidáció, log/javaslat rotáció, kurzuscsomag takarítás, biztonsági mentések korlátozott megőrzési ideje                         | 1 x 4 = alacsony    |
| 7   | Biztonsági naplók túl sok információt tartalmaznak vagy jogosulatlanul hozzáférhetők | IP cím, felhasználói azonosító és használati minták illetéktelen megismerése                                | 3 x 3 = közepes      | Célhoz kötött naplózás, megőrzési idő, admin / internal hozzáférés, logrotáció, MFA                                                                                                                   | 2 x 3 = közepes     |
...
| 10  | 16-18 éves felhasználók adatainak kezelése                                           | Kiskorú érintettek fokozottabb sérülékenysége                                                               | 2 x 4 = közepes      | Szolgáltatás 16 év felettieknek, 16 év alatti adat tudomásszerzés esetén törlés, önkéntes használat, nincs marketingprofilozás                                                                        | 1 x 4 = alacsony    |
...
## 18. Kiemelt fennmaradó kockázatok
...
## 19. Előzetes konzultáció szükségessége
...
| ----------------------- | -------------- | ---------------- | ----------- | ------------------------------------ |
| Adatkezelő              | Szőcs Barnabás | Jóváhagyott      | 2026.07.07. | Eljárt mint adatkezelő és fejlesztő. |
| Adatvédelmi tisztviselő | -              | Nem alkalmazandó | -           | Nincs kijelölt DPO.                  |

## 22. Felülvizsgálati napló
...
| 2026.06.13. | Harmadik verzió | "Admin jogosultság hibás kezelése vagy admin fiók kompromittálódása" kockázatértékelés frissítése (azonnali értesítések). |
| 2026.06.15. | Negyedik verzió | Kockázatok újraértékelése az admin MFA, illetve további biztonsági mechanizmusok bevezetése után.                         |
| 2026.07.07. | Ötödik verzió   | DPIA kiegészítése az EDPB releváns ajánlása alapján, "ping" adatkezelés módosításának dokumentálása                       |

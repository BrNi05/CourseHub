...
- Average calculator server-side saves live in `User.creditProfile` as JSON. The JSON copies course name, course code, and credit value; it must not link calculator entries to `Course` rows or course IDs.

"""
Command-line interface for the CourseHub database backup system.

This module provides a full CLI entrypoint for managing backups, including:
- Manual and scheduled backup execution
- Configuration management (cookie, schedule)
- LaunchAgent installation for macOS automation
- State inspection and logging utilities
"""
...
def build_parser() -> argparse.ArgumentParser:
    """
    Builds the CLI argument parser with all supported subcommands.
    """
    ...
    subparsers.add_parser("scheduled-run", help="Run only if backup is due.")

    set_cookie_parser = subparsers.add_parser(
        "set-cookie",
        help="Store authentication cookie.",
    )
    set_cookie_parser.add_argument("value", help="Cookie value.")

    subparsers.add_parser("clear-cookie", help="Remove stored cookie.")

    set_schedule_parser = subparsers.add_parser(
        "set-schedule",
        help="Update daily schedule and reload LaunchAgent if installed.",
    )
    ...
    subparsers.add_parser("install-agent", help="Install or update LaunchAgent.")
    subparsers.add_parser("uninstall-agent", help="Remove LaunchAgent.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    Dispatches commands to their respective handlers and manages
...
def _status(paths, config_store: ConfigStore, state_store: StateStore) -> int:
    """
    Displays full system status including configuration, state, and scheduling info.
    """
    ...
    print(f"Last size: {state.last_bytes} bytes ({state.last_bytes / (1024 * 1024):.3f} MB)")
    print(f"Last reason: {state.last_reason or 'n/a'}")
    print(f"Last error: {state.last_error or 'none'}\n")

    print(f"Schedule: {config.schedule_hour:02d}:{config.schedule_minute:02d} (local)")
    ...
    try:
        ...
        print(
            f"LaunchAgent loaded: {'yes' if agent_status.loaded else 'no'}"
        )
...
def _run_backup(backup_client: BackupClient, reason: str) -> int:
    """
    Executes a backup and prints the result.
    """
...
def _scheduled_run(
    ...
    """
    Runs backup only if scheduling logic determines it is due.
    """
    config = config_store.load()
    state = state_store.load()
    now = local_now()
    ...
    if not is_backup_due(now, config, state):
        logger.info("Scheduled run skipped at %s (not due).", now.isoformat())
        return 0
...
def _set_cookie(config_store: ConfigStore, value: str) -> int:
    """
    Stores authentication cookie in configuration.
    """
    config = config_store.load()
...
def _clear_cookie(config_store: ConfigStore) -> int:
    """
    Removes stored authentication cookie.
    """
    config = config_store.load()
...
def _set_schedule(paths, config_store: ConfigStore, hour: int, minute: int) -> int:
    """
    Updates backup schedule and reloads LaunchAgent if present.
    """
    config = config_store.load()
    ...
    try:
        if paths.launch_agent_path.exists():
            ...
            print("LaunchAgent reloaded with new schedule.")
    except RuntimeError as error:
...
def _show_logs(log_path: Path, lines: int) -> int:
    """
    Prints last N lines of the log file.
    """
...
def _install_agent(paths, config_store: ConfigStore) -> int:
    """
    Installs LaunchAgent for automatic scheduling.
    """
    config = config_store.load()
...
def _format_iso(value: str | None) -> str:
    """
    Formats ISO datetime string into human-readable local time.
    """
    if not value:
        return "never"
